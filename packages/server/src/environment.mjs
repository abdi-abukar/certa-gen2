...
export function environmentFor(app, settings, inherited = process.env) {
  ...
  if(app==='content') for(const name of ['SUPABASE_SECRET_KEY','RESEND_API_KEY','NEWSLETTER_DELIVERY_MODE','EMAIL_DELIVERY_MODE','EMAIL_FROM','EMAIL_REPLY_TO']) if(settings[name]) environment[name]=settings[name];
  if (app === 'web') {
    for (const name of [...commerceSettings, 'CERTA_AUTH_CHALLENGE_SECRET']) if (settings[name]) environment[name] = settings[name];
  }
  if (app === 'web' || app === 'admin') for (const name of ['CERTA_CHECKOUT_TERMS_VERSION','CERTA_CHECKOUT_TERMS_URL']) if (settings[name]) environment[name] = settings[name];
