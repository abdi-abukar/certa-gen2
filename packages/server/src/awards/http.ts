...
export async function awardsResponse(request:Request,segments:string[],staff=false){
 ...
 try{
  ...
  if(request.method==='GET'){
   ...
   if(path==='accounts'){let q=db.from('ct_accounts').select('id,kind,lifecycle,compliance_hold,vendor_updated_at').eq('user_id',owner).order('id').range(offset,offset+49);return reply({items:checked(await q)});}
   ...
   if(segments[0]==='accounts'&&segments.length===3&&segments[2]==='game'){
    const a=checked(await db.from('ct_accounts').select('id,user_id,kind,lifecycle,compliance_hold,vendor_updated_at,data').eq('id',uuid(segments[1])).eq('user_id',owner).single());
